...
const Experience = () => {
  ...
  const handleTabChange = (e) => {
    if (e.target.dataset.testid) {
      e.target.dataset.testid.includes('Left') ? setCurrentTab(currentTab - 1) : setCurrentTab(currentTab + 1)
    } else if (e.target.id) {
        ...
        <Fade>
          <Box color='primary' sx={{ ...cardStyling, display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, width: { xs: '90%', md: '80%' } }}>
            ...
            <Tabs
              ...
              onClick={handleTabChange}
              textColor='secondary'
              sx={{ display: { xs: 'flex', sm: 'none' }, borderRight: 1, borderColor: 'divider', width: '100%', color: 'primary.dark', mb: 3, overflow: 'scroll' }}
            >
